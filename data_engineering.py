...
def train_step(model: torch.nn.Module,
    ...
    acc /= len(data_loader)
    print('\n-----------')
    print(f'Mean Train Loss: {total_loss:.4f}, Mean Train Accuracy: {acc * 100:.2f}%')
...
def test_step(model: torch.nn.Module,
    ...
    with torch.inference_mode():
        ...
        total_loss /= len(data_loader)
        acc /= len(data_loader)
    print(f'Mean Test Loss: {total_loss:.4f}, Mean Test Accuracy: {acc * 100:.2f}%')
    print('-----------\n')
...
def accuracy_fn(y_pred, y_true) -> float:
    ...
    """
    return 1 - torch.nn.functional.mse_loss(target=y_true, input=y_pred).item()


def make_predictions(model: torch.nn.Module, samples: list) -> torch.Tensor():
    """
